...
static int box_map_local_entry(const Box &box, const int ghost, int local_x,
  ...
  if (0 <= local_x && local_x < nx && 0 <= local_y && local_y < ny &&
...
int box_map_local(const Box &box_local, const int ghost,
                  const int box_local_map[], const int local_x,
                  const int local_y, const int local_z) {
...
void box_partition(int ip, int up, int axis, const Box &box, Box *p_box) {
  const int np = up - ip;
  if (1 == np) {
    p_box[ip][0][0] = box[0][0];
    p_box[ip][0][1] = box[0][1];
    p_box[ip][1][0] = box[1][0];
    p_box[ip][1][1] = box[1][1];
    p_box[ip][2][0] = box[2][0];
    p_box[ip][2][1] = box[2][1];
  } else {
...
static int box_disjoint(const Box &a, const Box &b) {
  return a[0][1] <= b[0][0] || b[0][1] <= a[0][0] || a[1][1] <= b[1][0] ||
         b[1][1] <= a[1][0] || a[2][1] <= b[2][0] || b[2][1] <= a[2][0];
}
...
static void box_partition_maps(const int np, const int my_p, const Box *pbox,
                               const int ghost, int **map_local_id,
                               int **map_recv_pc, int **map_send_pc,
                               int **map_send_id) {
      ...
      for (local_z = -ghost; local_z < my_nz + ghost; ++local_z) {
        for (local_y = -ghost; local_y < my_ny + ghost; ++local_y) {
          for (local_x = -ghost; local_x < my_nx + ghost; ++local_x) {
            ...
            if (p_iz <= global_z && global_z < p_ez && p_iy <= global_y &&
                global_y < p_ey && p_ix <= global_x && global_x < p_ex) {
              ...
              if (my_p != ip) {
                fprintf(
                    stdout, "  (%d,%d,%d) : P%d recv at local %d from P%d\n",
            ...
            if (my_p != ip &&
                /* In my ownership: */
                (0 <= local_z && local_z < my_nz && 0 <= local_y &&
                 local_y < my_ny && 0 <= local_x && local_x < my_nx) &&
...
#if defined(DEBUG_PRINT)
              {
                fprintf(stdout, "  (%d,%d,%d) : P%d send at local %d to P%d\n",
                        global_x, global_y, global_z, my_p, local_id[entry],
                        ip);
                fflush(stdout);
              }
#endif
...
void box_partition_rcb(const int np, const int my_p, const Box &root_box,
                       const int ghost, Box **pbox, int **map_local_id,
                       int **map_recv_pc, int **map_send_pc,
                       int **map_send_id) {
  *pbox = new Box[np];
  ...
  box_partition_maps(np, my_p, *pbox, ghost, map_local_id, map_recv_pc,
...
static int box_contain(const Box &a, const Box &b) {
  return a[0][0] <= b[0][0] && b[0][1] <= a[0][1] && a[1][0] <= b[1][0] &&
         b[1][1] <= a[1][1] && a[2][0] <= b[2][0] && b[2][1] <= a[2][1];
}

static void box_print(FILE *fp, const Box &a) {
  fprintf(fp, "{ [ %d , %d ) , [ %d , %d ) , [ %d , %d ) }", a[0][0], a[0][1],
...
static void test_box(const Box &box, const int np) {
  ...
  fprintf(stdout, "NP = %d, total = %d, avg = %d, min = %d, max = %d\n", np,
          ncell_box, ncell_box / np, ncell_min, ncell_max);
}
...
static void test_maps(const Box &root_box, const int np) {
  ...
  for (ieq = i = 0; i < np; ++i) {
    ...
    box_partition_maps(np, i, &pbox[0], ghost, &map_local_id[i],
                       &map_recv_pc[i], &map_send_pc[i], &map_send_id[i]);
    ...
    for (iz = -ghost; iz < nz + ghost; ++iz) {
      for (iy = -ghost; iy < ny + ghost; ++iy) {
        for (ix = -ghost; ix < nx + ghost; ++ix) {
          const int ieq =
              box_map_local(mybox, ghost, map_local_id[i], ix, iy, iz);
          ...
          if (0 <= ieq) {
            ...
            if (root_box[0][0] <= ix_global && ix_global < root_box[0][1] &&
              ...
              local_values[i][ieq] = ix_global + iy_global * nx_global +
