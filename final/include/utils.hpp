...
Scalar percentage_difference(Ordinal value, Scalar average) {
  ...
  Scalar result = std::abs(static_cast<Scalar>(value - average));
  if (average != 0) {
    result /= static_cast<Scalar>(average);
    result *= 100;
  } else
    result = -1;
...
//
// Input argument local_nrows is really a integer, but taking it as a
// floating-point scalar is harmless.
//
#ifdef HAVE_MPI
  int numprocs = 1, myproc = 0;
  MPI_Comm_size(MPI_COMM_WORLD, &numprocs);
  ...
  // If it's significantly more efficient, we may consider using MPI_Gather
  // below instead of MPI_Allgather. We really only need to compute std.dev. on
  // proc 0...
  //
  ...
  // turn all_nrows contents into deviations, add to
...
find_row_for_id(GlobalOrdinal id,
  ...
  if (iter == ids_to_rows.begin() && iter->first > id) {
    std::cout << "ERROR, id:" << id << "ids_to_rows.begin(): " << iter->first
              << std::endl;
    return -99;
  }
