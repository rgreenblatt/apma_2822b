...
public:
  simple_mesh_description(const Box &global_box_in, const Box &local_box_in) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      // num-owned-nodes == num-elems+1 in this dimension if the elem box is not
      // empty and we are at the high end of the global range in that dimension:
      if (local_box_in[i][1] > local_box_in[i][0] &&
          local_box_in[i][1] == global_box[i][1])
        local_node_box[i][1] += 1;
    }
    ...
    if (global_box[X][0] == local_box[X][0]) {
      ...
      if (local_node_box[Y][0] > 0)
        --miny;
      if (local_node_box[Z][0] > 0)
        --minz;
      if (local_node_box[Y][1] < max_node_y)
        ++maxy;
      if (local_node_box[Z][1] < max_node_z)
        ++maxz;
    ...
    if (global_box[Y][0] == local_box[Y][0]) {
      ...
      if (local_node_box[X][0] > 0)
        --minx;
      if (local_node_box[Z][0] > 0)
        --minz;
      if (local_node_box[X][1] < max_node_x)
        ++maxx;
      if (local_node_box[Z][1] < max_node_z)
        ++maxz;
