...
KERNEL_PREFIX void gradients_and_detJ(const Scalar *elemNodeCoords,
                                      const Scalar *grad_vals, Scalar &detJ) {
  /**
...
KERNEL_PREFIX void diffusionMatrix_symm(const Scalar *elemNodeCoords,
  ...
  Scalar dpsidx[numNodesPerElem], dpsidy[numNodesPerElem],
      dpsidz[numNodesPerElem];
    ...
    for (size_t jg = 0; jg < numGaussPointsPerDim; ++jg) {
      ...
      for (size_t kg = 0; kg < numGaussPointsPerDim; ++kg) {
        ...
        for (int i = 0; i < numNodesPerElem; ++i) {
          ...
          dpsidx[i] = gv0 * invJ[0] + gv1 * invJ[1] + gv2 * invJ[2];
          dpsidy[i] = gv0 * invJ[3] + gv1 * invJ[4] + gv2 * invJ[5];
          dpsidz[i] = gv0 * invJ[6] + gv1 * invJ[7] + gv2 * invJ[8];
          gv += spatialDim;
        }
        ...
        for (int m = 0; m < numNodesPerElem; ++m) {
          ...
          elem_mat[offset++] +=
              k_detJ_wi_wj_wk * ((dpsidx_m * dpsidx_m) + (dpsidy_m * dpsidy_m) +
                                 (dpsidz_m * dpsidz_m));

          for (int n = m + 1; n < numNodesPerElem; ++n) {
            elem_mat[offset++] += k_detJ_wi_wj_wk * ((dpsidx_m * dpsidx[n]) +
                                                     (dpsidy_m * dpsidy[n]) +
                                                     (dpsidz_m * dpsidz[n]));
          }
        }
...
KERNEL_PREFIX void sourceVector(const Scalar *elemNodeCoords,
                                const Scalar *grad_vals, Scalar *elem_vec) {
