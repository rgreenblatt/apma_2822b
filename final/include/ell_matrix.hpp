...
struct ELLMatrix {
  ELLMatrix()
      : has_local_indices(false), rows(), cols(), coefs(), num_cols(0),
        num_cols_per_row(0)
#ifdef HAVE_MPI
        ,
  ...
  size_t num_nonzeros() const { return rows.size() * num_cols_per_row; }
  ...
  void get_row_pointers(GlobalOrdinalType row, size_t &row_length,
                        GlobalOrdinalType *&cols_ptr, ScalarType *&coefs_ptr) {
