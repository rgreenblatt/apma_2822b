...
void sort_if_needed(GlobalOrdinal *list, GlobalOrdinal list_len) {
...
template <>
struct MatrixInitOp<miniFE::CSRMatrix<MINIFE_SCALAR, MINIFE_LOCAL_ORDINAL,
  ...
  MatrixInitOp(
      ...
      miniFE::CSRMatrix<MINIFE_SCALAR, MINIFE_LOCAL_ORDINAL,
    ...
    MINIFE_LOCAL_ORDINAL nnz = row_offsets_vec[n];
    if (static_cast<MINIFE_LOCAL_ORDINAL>(matrix.packed_cols.capacity()) <
        nnz) {
      std::cout << "Warning, packed_cols.capacity ("
                << matrix.packed_cols.capacity()
                << ") < "
  ...
  inline void operator()(int i) {
    dest_rows[i] = rows[i];
    LocalOrdinalType offset = row_offsets[i];
