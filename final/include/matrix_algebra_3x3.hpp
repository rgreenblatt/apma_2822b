...
template <typename Scalar>
KERNEL_PREFIX void inverse_and_determinant3x3(const Scalar *J, Scalar *invJ,
                                              Scalar &detJ) {
...
KERNEL_PREFIX void matmat3x3(const Scalar *A, const Scalar *B, Scalar *C) {
  ...
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      C[i + j * 3] = A[i + 0] * B[j * 3 + 0] + A[i + 3] * B[j * 3 + 1] +
                     A[i + 6] * B[j * 3 + 2];
    }
  }
}
...
template <typename Scalar>
KERNEL_PREFIX void matmat3x3_X_3xn(const Scalar *A, int n, const Scalar *B,
                                   Scalar *C) {
...
template <typename Scalar>
KERNEL_PREFIX void matTransMat3x3_X_3xn(const Scalar *A, int n, const Scalar *B,
                                        Scalar *C) {
  // A is 3x3, B is 3xn. So C is also 3xn.
  // A,B,C are all assumed to be ordered such that columns are contiguous.
