...
void time_kernels(OperatorType &A, const VectorType &b, VectorType &x,
                  Matvec matvec,
  ...
  if (!A.has_local_indices) {
    std::cerr << "miniFE::time_kernels ERROR, A.has_local_indices is false, "
              << "needs to be true. This probably means "
  ...
  // cases, but perform actual allocations and copies if a off-cpu device such
  ...
  OrdinalType *d_Arowoff =
      compute_node.get_buffer(&A.row_offsets[0], A.row_offsets.size());
  OrdinalType *d_Acols =
      compute_node.get_buffer(&A.packed_cols[0], A.packed_cols.size());
  ScalarType *d_Acoefs =
      compute_node.get_buffer(&A.packed_coefs[0], A.packed_coefs.size());
  ...
  compute_node.copy_to_buffer(&A.row_offsets[0], A.row_offsets.size(),
                              d_Arowoff);
  compute_node.copy_to_buffer(&A.packed_cols[0], A.packed_cols.size(), d_Acols);
  compute_node.copy_to_buffer(&A.packed_coefs[0], A.packed_coefs.size(),
                              d_Acoefs);
