...
//  The assembled linear system will have 1 equation for each finite element
//  node.
//
...
int driver(const Box &global_box, Box &my_box, Parameters &params,
  ...
  RUN_TIMED_FUNCTION("assembling FE data...",
                     assemble_FE_data(mesh, A, b, params), fe_assembly,
                     t_total);

  if (myproc == 0) {
    ydoc.add("Matrix structure generation", "");
    ydoc.get("Matrix structure generation")
        ->add("Mat-struc-gen Time", gen_structure);
  ...
  RUN_TIMED_FUNCTION("making matrix indices local...", make_local_matrix(A),
    ...
    if (params.verify_solution > 0) {
      ...
      if (myproc == 0) {
        if (verify_whole_domain)
          ...
          std::cout << "verifying solution at ~ (0.5, 0.5, 0.5) ..."
                    << std::endl;
      }
  ...
  if (myproc == 0) {
    ...
    double mv_flops = static_cast<double>(global_nnz) * 2.0;
