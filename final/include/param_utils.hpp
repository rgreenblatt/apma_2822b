 ...
 */
template <typename T>
T parse_parameter(const std::string &arg_string, const std::string &param_name,
  ...
  if (arg_string.size() <= pos)
    return default_value;

  // skip past ' ', '=' or ':':
  while (pos < arg_string.size() &&
         (arg_string[pos] == ' ' || arg_string[pos] == '=' ||
          arg_string[pos] == ':')) {
    ++pos;
  }

  if (arg_string[pos] == '=' || arg_string[pos] == ':')
    ++pos;
