...
int generate_matrix_structure(
  ...
  try {
    ...
    try {
      ...
      osstr << "One of A.rows.resize, A.row_offsets.resize, "
               "A.packed_cols.reserve or A.packed_coefs.reserve: nrows="
            << nrows << ": ";
      osstr << exc.what();
    ...
    std::cout << "proc " << myproc
              << " threw an exception in generate_matrix_structure, probably "
