...
void get_hex8_node_ids(int nx, int ny, GlobalOrdinal node0,
...
void get_hex8_node_coords_3d(Scalar x, Scalar y, Scalar z, Scalar hx, Scalar hy,
                             Scalar hz, Scalar *elem_node_coords) {
...
void get_elem_nodes_and_coords(
    const simple_mesh_description<GlobalOrdinal> &mesh, GlobalOrdinal elemID,
  ...
  if (elemID < 0) {
    ...
    throw std::runtime_error(
        "get_elem_nodes_and_coords ERROR, negative elemID");
  }
  ...
  // Map node-IDs to rows because each processor may have a non-contiguous block
  // of node-ids, but needs a contiguous block of row-numbers:
#ifdef MINIFE_DEBUG_VERBOSE
  ...
  Scalar ix, iy, iz;
  get_coords<GlobalOrdinal, Scalar>(nodeID, global_nodes_x, global_nodes_y,
  ...
  int offset = 0;
  for (int i = 0; i < Hex8::numNodesPerElem; ++i) {
    std::cout << "(" << node_coords[offset++] << "," << node_coords[offset++]
              << "," << node_coords[offset++] << ")";
  }
  std::cout << std::endl;
#endif
