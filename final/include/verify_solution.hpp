...
template <typename Scalar> struct err_info {
  Scalar err;
  Scalar computed;
  Scalar analytic;
  Scalar coords[3];
...
int verify_solution(const simple_mesh_description &mesh, const VectorType &x,
  ...
  for (int iz = box[2][0]; iz < box[2][1]; ++iz) {
    for (int iy = box[1][0]; iy < box[1][1]; ++iy) {
      for (int ix = box[0][0]; ix < box[0][1]; ++ix) {
        int row_id =
            get_id(global_nodes_x, global_nodes_y, global_nodes_z, ix, iy, iz);
        Scalar x, y, z;
        ...
        else if (std::abs(x - 0.5) < 0.05 && std::abs(y - 0.5) < 0.05 &&
                 std::abs(z - 0.5) < 0.05) {
          verify_this_point = true;
        }
  ...
  for (size_t i = 0; i < rows.size(); ++i) {
    ...
    std::cout << "(" << x << "," << y << "," << z << ") row " << rows[i]
              << ": computed: " << computed_soln
              << ",  analytic: " << analytic_soln << std::endl;
#endif
  ...
  if (local_max_err == global_max_err) {
    if (max_error.err > tolerance) {
      std::cout << "max absolute error is " << max_error.err << ":"
                << std::endl;
      std::cout << "   at position (" << max_error.coords[0] << ","
                << max_error.coords[1] << "," << max_error.coords[2] << "), "
                << std::endl;
      std::cout << "   computed solution: " << max_error.computed
                << ",  analytic solution: " << max_error.analytic << std::endl;
    } else {
      std::cout << "solution matches analytic solution to within " << tolerance
                << " or better." << std::endl;
    }
  }
