...
void compute_imbalance(const Box &local_box, double &largest_imbalance,
  ...
  if (myproc == 0 && record_in_doc) {
    ...
    doc.get("Rows-per-proc Load Imbalance")
        ->add("Largest (from avg, %)", largest_imbalance);
...
void add_imbalance(const Box &global_box, Box &local_box, float imbalance,
  ...
  while (cur_imbalance < imbalance) {
    ...
    int grow_info[8] = {grow.first,      grow.second,     local_box[X][0],
                        local_box[X][1], local_box[Y][0], local_box[Y][1],
                        local_box[Z][0], local_box[Z][1]};

    int shrink_info[8] = {shrink.first,    shrink.second,   local_box[X][0],
                          local_box[X][1], local_box[Y][0], local_box[Y][1],
                          local_box[Z][0], local_box[Z][1]};
#ifdef HAVE_MPI
    ...
    if (shrink_end == LOWER)
      shrink_incr = 1;
    if (grow_axis != NONE)
      grow_axis_val = grow_info[2 + grow_axis * 2 + grow_end];
    if (shrink_axis != NONE)
      shrink_axis_val = shrink_info[2 + shrink_axis * 2 + shrink_end];

    if (grow_axis == NONE && shrink_axis == NONE)
      break;
    ...
    MPI_Allreduce(&statusints, &globalstatus, 2, MPI_INT, MPI_SUM,
                  MPI_COMM_WORLD);
    grow_status = globalstatus[0] > 0 ? false : true;
    shrink_status = globalstatus[1] > 0 ? false : true;
#endif

    if (grow_status == false && shrink_status == false)
      break;
